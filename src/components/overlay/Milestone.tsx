...
export function Milestone({
  ...
  return (
    <div className="w-[340px] font-sans">
      <div className="bg-white rounded-[2rem] p-6 border border-purple-50 relative overflow-hidden">
        {/* Header */}
        <div className="flex items-center gap-4 mb-6 relative z-10">
          <div className="bg-yellow-100 p-2.5 rounded-2xl text-yellow-600 transform -rotate-3">
            <Trophy size={20} fill="currentColor" className="text-yellow-500" />
          </div>
          <div>
            <h2 className="text-xl font-black text-purple-900 leading-none tracking-tight">
              {title}
            </h2>
            <span className="text-[10px] font-bold text-slate-400 tracking-[0.2em] uppercase mt-1 block">
              {subtitle}
            </span>
          </div>
        </div>
        ...
        <div className="relative pl-2 space-y-6 z-10">
          ...
          {sortedSteps.map((step, idx) => {
            ...
            const isActive =
              !isCompleted && (idx === 0 || currentValue >= sortedSteps[idx - 1].value);
            ...
            const progressPercent = isActive
              ? Math.min(100, (currentInStep / range) * 100)
              : isCompleted
                ? 100
                : 0;
            ...
            return (
              <div key={idx} className="relative pl-10">
                ...
                <div
                  className={cn(
                    ...
                    isCompleted
                      ...
                      : isActive
                        ? "border-yellow-100 bg-yellow-50 scale-105"
                        : "border-slate-50 bg-white"
                  )}
                >
                  {isCompleted && <div className="w-2.5 h-2.5 bg-purple-300 rounded-full" />}
                  {isActive && (
                    <div className="w-3 h-3 rounded-full border-[2.5px] border-yellow-400" />
                  )}
                ...
                <div
                  className={cn(
                    "flex ml-3 flex-col gap-1 transition-all duration-500",
                  ...
                  <div className="flex justify-between items-baseline">
                    <h3
                      className={cn(
                        ...
                        isCompleted
                          ? "text-slate-300 line-through decoration-2 decoration-purple-200"
                          : isActive
                            ? "text-purple-700"
                            : "text-slate-400"
                      )}
                    ...
                    {isActive && showAmounts && (
                      <span className="text-[10px] font-black text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded-md">
                  ...
                  {isCompleted ? (
                    ...
                    showAmounts && (
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">
                        TARGET:{" "}
                        <span className="text-slate-500">{step.value.toLocaleString()}</span>
                      </span>
                    )
                  )}
