...
export function RecentActivityWidget() {
    const [activities, setActivities] = useState<ActivityItem[]>([
        { user: "Budi Santoso", action: "donated", amount: 50000, time: new Date(Date.now() - 1000 * 60 * 2) },
        { user: "Siti Aminah", action: "donated", amount: 100000, time: new Date(Date.now() - 1000 * 60 * 5) },
        { user: "Joko", action: "donated", amount: 10000, time: new Date(Date.now() - 1000 * 60 * 60 * 24 * 8) },
    ]);
                    ...
                    <div key={idx} className="p-3 flex items-start gap-3 hover:bg-slate-50 transition-colors">
                        ...
                        <div>
                            <p className="text-xs text-slate-600">
                                <span className="font-bold text-slate-900">{activity.user}</span> donated <span className="font-bold text-[var(--color-deep-purple)]">
                                    Rp {activity.amount.toLocaleString('id-ID')}
                                </span>
                            </p>
                            <p className="text-[10px] text-slate-400 font-bold mt-0.5">{formatTime(activity.time)}</p>
                        </div>
                    </div>
                ))}
            </div>
