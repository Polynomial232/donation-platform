...
function BrandBadge() {
  const APP_NAME = process.env.NEXT_PUBLIC_APP_NAME || "Royal Kingdom";
  return (
    <div className="relative shrink-0 w-full mb-6">
      <div className="flex items-center p-1 w-full">
        <div className="h-10 w-10 flex items-center justify-center p-2 bg-slate-100 rounded-xl shadow-sm ring-1 ring-slate-200">
          <span className="text-slate-900 font-black text-lg">{APP_NAME.charAt(0)}</span>
        </div>
        <div className="px-2 py-1">
          <div className="font-sans text-[16px] text-slate-100 font-bold tracking-tight">
            {APP_NAME}
          </div>
        </div>
      </div>
...
function SearchContainer({
  ...
  return (
    ...
    >
      <div
        className={`bg-neutral-900/50 h-10 relative rounded-xl flex items-center transition-all duration-500 ring-1 ring-neutral-800/50 ${
          ...
          <div className="flex flex-col justify-center size-full">
            <div className="flex flex-col gap-2 items-start justify-center pr-2 py-1 w-full">
              <input
                ...
                className="w-full bg-transparent border-none outline-none font-sans text-[13px] text-neutral-50 placeholder:text-neutral-500 leading-tight"
...
function IconNavButton({
  ...
  return (
    <button
      type="button"
      className={cn(
        "flex items-center justify-center rounded-xl size-10 min-w-10 transition-all duration-500 group",
        isActive
...
export function IconNavigation({
  ...
  return (
    <aside className="bg-slate-900 flex flex-col gap-3 items-center p-4 w-18 h-full border-r border-slate-800/50 backdrop-blur-xl">
      {/* Branding - Mini */}
      <div className="mb-4 size-10 flex items-center justify-center bg-zinc-900 rounded-xl ring-1 ring-white/5 shadow-2xl">
      ...
      {/* Bottom section */}
      <div className="flex flex-col gap-4 w-full items-center pb-2">{bottomItems}</div>
    </aside>
  );
}
...
function SectionTitle({
    ...
    return (
      ...
      >
        <button
          ...
          className="flex items-center justify-center rounded-xl size-10 min-w-10 transition-all duration-500 hover:bg-neutral-800 text-neutral-400 hover:text-neutral-50"
          style={{ transitionTimingFunction: softSpringEasing }}
  ...
  return (
    <div
      className="w-full overflow-hidden transition-all duration-500 mb-2"
      style={{ transitionTimingFunction: softSpringEasing }}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center h-10">
          <div className="px-2 py-1">
            <div className="font-sans text-[18px] text-slate-100 leading-[27px] font-bold tracking-tight">
              {title}
            </div>
          </div>
        </div>
        <div className="pr-1">
          <button
            type="button"
            onClick={onToggleCollapse}
            className="flex items-center justify-center rounded-xl size-10 min-w-10 transition-all duration-500 hover:bg-slate-800 text-slate-400 hover:text-white"
            style={{ transitionTimingFunction: softSpringEasing }}
...
export function DetailSidebar({
  ...
  return (
    <aside
      className={cn(
        "bg-slate-900 flex flex-col gap-4 items-start p-4 transition-all duration-500 h-full border-r border-slate-800/50 backdrop-blur-xl relative z-10",
    ...
    >
      ...
      />
      <SearchContainer isCollapsed={isCollapsed} value={searchValue} onChange={setSearchValue} />

      <div
        className={cn(
          "flex flex-col w-full overflow-y-auto no-scrollbar flex-1 transition-all duration-500 mt-2",
      ...
      {!isCollapsed && (
        <div className="w-full mt-auto pt-4 border-t border-slate-800/50">
          <div className="flex items-center gap-3 px-2 py-2 group cursor-pointer hover:bg-slate-800/50 rounded-xl transition-all">
            <AvatarCircle />
            <div className="flex flex-col">
              <div className="font-sans text-[13.5px] text-slate-100 font-semibold leading-none mb-1 group-hover:text-white">
                Account Profile
              </div>
              <div className="font-mono text-[9px] text-slate-500 font-medium uppercase tracking-wider">
                Dashboard User
              </div>
            </div>
          </div>
...
function MenuItem({
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    else if (item.href) onItemClick?.(item.href);
  };
  ...
  return (
    ...
    >
      <div
        className={cn(
          "rounded-xl cursor-pointer transition-all duration-500 flex items-center relative group",
          item.isActive
            ? "bg-slate-700 text-white shadow-lg"
            : "hover:bg-slate-800/60 text-slate-400 hover:text-white",
          isCollapsed ? "w-10 min-w-10 h-10 justify-center" : "w-full h-11 px-4 py-2"
        )}
        style={{ transitionTimingFunction: softSpringEasing }}
        onClick={handleClick}
      ...
      >
        ...
        <div
          ...
          style={{ transitionTimingFunction: softSpringEasing }}
        >
          <div className="font-sans text-[14px] font-semibold tracking-tight">{item.label}</div>
        </div>
...
function SubMenuItem({
  ...
  return (
    <div className="w-full pl-10 pr-1 py-0.5">
      <div
        className={cn(
          "h-9 w-full rounded-lg cursor-pointer transition-all flex items-center px-3 py-1 text-neutral-400 hover:text-neutral-50 hover:bg-neutral-900/50",
          item.isActive && "text-white bg-neutral-900"
        )}
        onClick={() => item.href && onItemClick?.(item.href)}
      >
        <div className="flex-1 min-w-0">
          <div className="font-sans text-[12.5px] font-medium tracking-tight">{item.label}</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col w-full">
      <div
        ...
        style={{ transitionTimingFunction: softSpringEasing }}
      >
        <div className="flex items-center h-full px-4 text-neutral-500 font-medium tracking-widest uppercase text-[10px]">
          {section.title}
        </div>
      </div>
