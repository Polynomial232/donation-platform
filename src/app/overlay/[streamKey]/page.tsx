...
import { EventFeed, EventFeedItem } from "@/components/overlay/EventFeed";
...
const MOCK_EVENTS: EventFeedItem[] = [
  {
    id: "1",
    user: "Sultan_Gaming",
    action: "Just donated!",
    type: "donation",
    amount: "500K",
    message: "Semangat bang live nya!",
  },
  {
    id: "2",
    user: "Luna_Star22",
    action: "Resubscribed for 3 months",
    type: "subscription",
    count: "Sub x3",
  },
  { id: "3", user: "New_Viewer_01", action: "Started following", type: "follow" },
  { id: "4", user: "CoffeeAddict", action: "Donated", type: "donation", amount: "10K" },
  { id: "5", user: "IndoGamer_X", action: "Started following", type: "follow" },
];
...
export default function OverlayPage() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (Math.random() > 0.7) {
        ...
        if (Math.random() > 0.5) {
          // 50% chance it's a donation event
          setAlert({
            type: "donation",
            user: user,
            amount: amount,
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-transparent font-sans">
      ...
      {/* Left Sidebar: Subathon, Goal, Milestone, QR */}
      <div className="absolute top-4 left-6 bottom-20 w-[450px] flex flex-col gap-8 overflow-y-auto scrollbar-hide py-2 px-4">
        <Goal title="New Gaming Chair" current={1200000} target={2500000} />
        <Donathon
          startTime={Date.now()}
          endTime={endTime}
      ...
      {/* Right Sidebar: Leaderboard, Poll, Event Feed */}
      <div className="absolute top-4 right-4 bottom-22 w-[360px] flex flex-col items-end gap-4 overflow-y-auto scrollbar-hide py-2">
