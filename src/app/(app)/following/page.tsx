                ...
                <div className="space-y-4">
                        ...
                        <Link href={`/${creator.username}`} key={idx} className="block group">
                            <Card className="p-4 flex items-center justify-between border-none shadow-sm hover:bg-slate-50 transition-colors cursor-pointer">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-900 group-hover:text-[var(--color-deep-purple)] transition-colors">{creator.username}</h3>
                                        <p className={`text-xs font-bold ${creator.status === 'Live Now' ? 'text-red-500 animate-pulse' : 'text-slate-400'}`}>
                                            {creator.status}
                                        </p>
                                    </div>
                                </div>
                                <Button size="sm" variant="outline" className="rounded-xl font-bold text-xs pointer-events-none">
                                    Support
                                </Button>
                            </Card>
                        </Link>
                    ...
                    {following.length === 0 && (
                        <div className="text-center py-10 text-slate-400">
                            ...
                            <Link href="/explore" className="text-[var(--color-deep-purple)] font-bold underline">Find creators</Link>
                        </div>
                    )}
                </div>
            </div>
