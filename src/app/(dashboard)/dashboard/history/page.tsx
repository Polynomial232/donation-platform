...
export default function SupportHistoryPage({
  ...
  const transactions: SupportTransaction[] = [
    {
      ...
      date: new Date(),
      isAnonymous: false,
      media: { type: "sound", name: "Jumpscare", url: "#" },
    },
    {
      id: "TX-002",
      supporter: "Siti Aminah",
      email: "siti.aminah@yahoo.com",
      amount: 100000,
      message: "Request lagu galau dong.",
    ...
    {
      id: "TX-004",
      supporter: "GamerSejati",
      email: "gamer.sejati@game.net",
      amount: 10000,
      message: "GGWP!",
      date: new Date(Date.now() - 86400000),
      isAnonymous: false,
      media: { type: "video", name: "Meme Lucu", url: "#" },
    },
    {
      id: "TX-005",
      supporter: "FansBerat",
      email: "fans.berat@fansclub.id",
      amount: 200000,
      message: "Top up buat beli skin baru.",
      date: new Date(Date.now() - 172800000),
      isAnonymous: false,
    },
    {
      id: "TX-006",
      supporter: "Donatur Hati",
      email: "donatur.hati@charity.org",
      amount: 500000,
      message: "Berkah selalu ya.",
      date: new Date(Date.now() - 259200000),
      isAnonymous: false,
      media: { type: "sound", name: "Applause", url: "#" },
    },
    {
      id: "TX-007",
      supporter: "KucingOren",
      email: "kucing.oren@pets.com",
      amount: 15000,
      message: "Meow.",
    ...
    },
    {
      id: "TX-008",
      supporter: "User123",
      email: "user123@platform.com",
      amount: 30000,
      message: "Semoga terbantu.",
  ...
  const topSupporters = React.useMemo(() => {
    ...
    const recentTransactions = transactions.filter(
      (t) => moment(t.date).isAfter(oneMonthAgo) && !t.isAnonymous
    );

    // 2. Group by supporter and sum amounts
    const supporterMap = new Map<
      string,
      { name: string; email?: string; totalAmount: number; count: number }
    >();
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Pantau terus dukungan yang masuk dari para penggemarmu.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <Card className="col-span-1 p-6 border-none shadow-sm flex flex-col justify-center items-center text-center bg-gradient-to-br from-purple-50 to-white">
          ...
          <h3 className="text-sm font-bold text-slate-500 mb-1 uppercase tracking-wider">
            Dukungan Hari Ini
          </h3>
        ...
        <Card className="p-6 border-none shadow-sm flex flex-col gap-4 bg-yellow-50/50">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-bold text-slate-900">
                Donasi Tersemat ({pinnedTransactions.length}/3)
              </h3>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {pinnedTransactions.map((item) => (
              <div
                key={item.id}
                className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col gap-2 relative group"
              >
                <Button
                  variant="ghost"
                  size="sm"
                ...
                <p className="text-2xl font-extrabold text-[var(--color-deep-purple)]">
                        ...
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {item.message && !item.isAnonymous && (
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-8 w-8 p-0 rounded-full text-slate-400 hover:text-[var(--color-primary)] hover:bg-[var(--color-primary)]/10"
                            >
      ...
      <Card className="p-6 border-none shadow-sm flex flex-col gap-4">
        <div className="flex justify-between items-center">
          ...
          <p className="text-xs text-slate-500 font-medium bg-slate-100 px-2 py-1 rounded-md">
            1 Bulan Terakhir
          </p>
        </div>
